...
function buildPhasePlanningPreparedInstruction(
  ...
  return [
    ...
    "",
    "Treat phase foundation as both host environment and repository foundation when applicable. A foundation, engineering-foundation, build-baseline, repository-foundation, bootstrap, or equivalent phase cannot silently treat required development tooling as Operator-provided.",
    "When required development capabilities are unverified or missing, create candidate work that establishes and verifies them before dependent implementation work. Environment/toolchain establishment should appear at or before the first candidate that needs those capabilities, and may be combined with repository baseline work only when that remains one coherent bounded outcome.",
    "Distinguish managed capability, external capability, and human-interaction boundary: managed capability means ChampCity provisions and verifies it; external capability means approved evidence establishes outside ownership and absence blocks; human-interaction boundary means ChampCity prepares the action, requests only the necessary interaction, then resumes.",
    "Do not use phrases like prerequisite problem or Operator must install merely because ordinary development tooling is missing.",
    "",
    ...
    "The Phase Planning body must describe any required machine readiness, repository readiness, external capability ownership, and human-interaction boundaries that affect this phase.",
    "",
    ...
    "For candidate sequencing, establish and verify missing or unverified managed development capabilities before candidates that depend on them.",
    "",
