...
function buildProjectPlanningHandoffBody(context: ReturnType<typeof requireReadyProjectPlanningContext>): string {
  return [
    ...
    "- Treat the approved Project Intake and approved Architect Interview as semantic planning evidence. The prior Interview Prompt is provenance and contract evidence after Interview approval and requires semantic inspection only to resolve a contradiction or ambiguity.",
    ...
    "- Plan architecture or migration requirements and product or Operator capability requirements as simultaneous axes; neither axis may silently erase the other.",
    "",
    ...
    "- For every material product capability or workflow established by approved evidence, sequence it into a specific roadmap phase or outcome, explicitly defer it with rationale, explicitly supersede it with the governing replacement decision, or mark it conditional and state the condition. Silent omission is prohibited.",
    "- Distinguish user or product capability delivery from internal enabling services, interfaces, adapters, or equivalent boundaries; name the consuming behavior or outcome that proves capability delivery.",
    "- Each phase must identify, when applicable, both its architecture or migration boundary and the meaningful product or Operator capability made new, materially improved, or operationally usable.",
    ...
    "- For existing-product migrations or refactors that introduce a materially new deployment mode or major capability, exercise that capability through a real consuming vertical slice as early as architecture dependencies safely permit.",
