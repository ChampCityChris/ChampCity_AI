...
export interface WorkCardClosePresentationEvidence {
  path: string;
  displayName: string;
  disposition: string;
  readState: string;
...
export function WorkCardClosePresentation({
  actionError,
  actionFeedback,
  actionLabel,
  actingLabel,
  ...
  ariaLabel: string;
  canAct: boolean;
  evidence: WorkCardClosePresentationEvidence[];
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden" aria-label={ariaLabel}>
          ...
          <section className="rounded-lg border border-border bg-card p-3.5 text-card-foreground" aria-label="Close projection">
            <div className="mb-3 flex flex-wrap items-start justify-between gap-3 border-b border-border pb-3">
              <div className="min-w-0">
                ...
                <h2 className="m-0 break-words text-[18px] font-semibold text-foreground">{title}</h2>
                <p className="mt-1 break-all font-mono text-[12px] text-muted-foreground">{identityLine}</p>
              </div>
              <div className="rounded border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1.5 text-[12px] font-medium text-emerald-500">{stateLabel}</div>
            </div>

            <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
              {fields.map((field) => <CloseField key={field.label} label={field.label} value={field.value} />)}
              <CloseField label="Projection Action" value={projectionAction} />
            </div>
            <div className="mt-3 border-t border-border pt-3">
              <p className="mb-0.5 font-mono text-[11px] uppercase tracking-widest text-muted-foreground">Reason</p>
              <p className="break-words text-[13px] text-foreground">{reason}</p>
            </div>
          </section>
                ...
                {evidence.map((entry) => (
                  <div className="rounded border border-border bg-muted px-3 py-2" key={entry.path}>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <span className="font-mono text-[11px] uppercase tracking-widest text-muted-foreground">{entry.disposition} / {entry.readState}</span>
                    </div>
          ...
          <section className="rounded-lg border border-border bg-card p-3.5 text-card-foreground" aria-label="Close return action">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="break-words text-[13px] text-foreground">{nextWorkspace}</p>
              </div>
              <button
                ...
                type="button"
              >
                {isActing ? <CheckCircle2 aria-hidden="true" size={16} /> : <ArrowRight aria-hidden="true" size={16} />}
...
export function WorkCardCloseWorkspace({
  ...
  const evidence = (model?.sourceEvidence ?? []).map((evidencePath) => {
  ...
  const canReturn = Boolean(projection?.closed) && projection?.returnTarget === "phase-work-card-selection";
  return (
    <WorkCardClosePresentation
      actionError={actionError}
      actionFeedback={actionFeedback}
      actionLabel="Return to Phase Building / Next Work Card"
      actingLabel="Returning..."
      ariaLabel="Work Card Close workspace"
      canAct={canReturn}
      evidence={evidence}
      fields={[
        { label: "Phase ID", value: model?.currentPhaseId ?? "Waiting" },
        { label: "Work Card ID", value: model?.currentWorkCardId ?? "Waiting" },
        { label: "Return Target", value: projection?.returnTarget ?? "Waiting for close projection" },
      ]}
      identityLine={`${model?.currentPhaseId ?? "phase unresolved"} / ${model?.currentWorkCardId ?? "work card unresolved"}`}
      isActing={isReturning}
      nextWorkspace={canReturn ? "Work Card Map" : projection?.reason ?? "Current close evidence is not ready."}
      onAction={onReturnToSelection}
      projectionAction={projectionResult?.action ?? "Waiting"}
      reason={projection?.reason ?? "Close projection is loading."}
      stateLabel={projection?.closed ? "Closed" : "Not Closed"}
      title={`${model?.currentWorkCardId ?? "Work Card"} - ${model?.executionContext.workCard.title ?? model?.currentTarget ?? "Close / Next Work Card"}`}
    />
  );
}

export function workCardCloseProjectionFromResult(result: RuntimeActionResult | null): WorkCardCloseProjection | null {
  if (!result || !isRecord(result.payload)) {
    return null;
  }
  const { closed, returnTarget, reason } = result.payload;
  if (typeof closed !== "boolean" || typeof returnTarget !== "string" || typeof reason !== "string") {
    return null;
  }
