...
Sections may be marked not applicable when justified. The Architect may add narrowly relevant sections.

The JSON sibling must provide the same substantive information in structured fields and include:
...
The prompt must state that conversation text is not the durable record. Completion requires the substantive synchronized Markdown/JSON pair in the repository.

### Prompt regeneration

Preserve existing revision and invalidation behavior:

- revising Project Intake regenerates the prompt with the new source revision;
- prompt revision increments;
- an existing approved downstream interview becomes Pending/stale according to the current revision contract;
- no placeholder Project Architect Interview pair is created by the application.
