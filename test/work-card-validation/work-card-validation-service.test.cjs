...
test("advisory Architect prompt is generated from repository evidence", () => {
  ...
  assert.match(result.instruction, /Inconclusive means available evidence is insufficient/);
  assert.match(result.instruction, /## Blocking Findings section may state None/);
