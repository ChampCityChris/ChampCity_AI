...
test('real Integration Repair provider binds immutable policy changed paths ownership and governing bytes',async t=>{
 ...
 const routeDecisionId='decision-fixture',completion={kind:'plan',routeDecisionId,completionId:'PLAN01',revision:1,fingerprint:projectPlanExecution(plan).fingerprint,sourcePath:'planning/plan.md'};
 writeDoc(root,'planning/intake.md','work-intake','Pending',{identity:{intakeId:'fixture'}});writeDoc(root,'planning/plan.md','work-planning-plan','Approved',{identity:{intakeId:'fixture',routeDecisionId,planId:'PLAN01'}});
 const record={repositoryId:'fixture',intakeId:'fixture',incomingCommit:incoming,incomingBranch:branch,targetBranch:'main',baseCommit:base,completion,targetCommit:base,mergeBase:base,candidateId,conflictingPaths:[]};
 const provider=createIntegrationRepairPolicyProvider({repositoryRoot:root,repositoryId:'fixture',load:async()=>({binding,completion,intakePath:'planning/intake.md'})}).repairPolicy;
 ...
 const recreated=createIntegrationRepairPolicyProvider({repositoryRoot:root,repositoryId:'fixture',load:async()=>({binding,completion,intakePath:'planning/intake.md'})}).repairPolicy;
