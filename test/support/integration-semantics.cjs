...
function installSemanticSourceFixture(t, { verifyCheckpoints = false } = {}){
 ...
 if (!verifyCheckpoints) t.mock.method(branchModule,'createWorkIntakeBranchService',({repositoryRoot:root})=>({verify:async binding=>{
  if(git(root,'branch','--show-current')!==binding.workBranch||git(root,'rev-parse','HEAD')!==binding.currentHead)throw Error('Fixture branch changed');return binding;
 }}));
 t.mock.method(sourceModule,'createSourceControlService',({repositoryRoot:root,repositoryId})=>{
  ...
  const result={status:()=>run('status',()=>({clean:git(root,'status','--porcelain')===''})),fetch:remote=>run('fetch',()=>git(root,'fetch',remote))};
  ...
  result.history=(input={})=>run('history',()=>{const commits=git(root,'log','-'+(input.maxCount??100),'--format=%H%x09%P%x09%s',input.ref??'HEAD').split('\n').map(line=>{const [commit,parents,subject]=line.split('\t');return{commit,parents:parents?parents.split(' '):[],subject}});let ancestry;if(input.ancestor){let isAncestor=true;try{git(root,'merge-base','--is-ancestor',input.ancestor,input.descendant)}catch{isAncestor=false}ancestry={isAncestor}}return{commits,ancestry}});
