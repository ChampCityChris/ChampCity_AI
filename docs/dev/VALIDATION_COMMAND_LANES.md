...
ChampCity_AI has a known false-failure mode where sandboxed Codex execution may block child-process spawning. This commonly appears as `spawn EPERM`, especially when tools such as esbuild, Vite, Vitest, Electron, Playwright, or build pipelines try to spawn subprocesses.
