...
Git branch, commit, push, and tag mechanics remain the exclusive responsibility of `git_toolbox`. `release_toolbox.publish_github_release` never creates or pushes a tag: it fails closed unless the exact `v<packageVersion>` tag exists locally at current `HEAD` and the same target exists on `origin`. GitHub publication uses the installed, authenticated `gh` CLI and separates release-metadata creation from canonical-installer upload. Before either mutation it inspects the provider release and requires the application-derived tag, title, prerelease state, published state, and newline-normalized canonical notes. An absent release receives metadata only and is re-inspected; an exact release with zero assets receives only the canonical installer; an exact release with exactly that installer is an idempotent completion requiring no mutation. Any mismatch, draft, unexpected or duplicate asset, multiple assets, or unparsable state fails closed without editing, deleting, replacing, or clobbering provider content. Provider reads and metadata creation have short fixed deadlines; the separate large-asset upload has a fixed 30-minute deadline. A failed or timed-out caller can explicitly retry, causing the provider state to be classified again from scratch. Post-upload success requires another exact inspection, while `verify_github_release` remains mandatory and separately downloads the canonical asset to OS temporary storage for exact SHA-256 comparison and cleanup.
