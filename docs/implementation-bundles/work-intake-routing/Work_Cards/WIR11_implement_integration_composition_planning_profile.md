...
**Implementer Report:** `docs/implementation-bundles/work-intake-routing/Implementer_Reports/WIR11_IMPLEMENTER_REPORT.md`
...
- Integration-heavy projects are poorly represented by greenfield build-first planning.
- Architecture requires build-vs-integrate disposition, ownership/contracts, lifecycle/failure, version risk, and gap-driven code.
...
Create planning for work primarily composed from existing components/services, with custom code limited to demonstrated gaps/adapters.

## Runtime / Workflow Sequence

```text
verify dependencies/current source
→ implement only WIR11
→ focused validation
→ write WIR11 report
→ checkpoint commit
→ read WIR12 from repository
```

## Required Changes

1. Define evidence/discovery for components, contracts, auth/access, data/control flow, lifecycle/health, version risk, adapters, demonstrated gaps.
2. Require build-vs-integrate disposition per material capability.
3. Allow Work Items for configure/characterize/integrate/adapt/validate/replace/reject.
4. Keep topology evidence-driven.

## Preserved Behavior

- Shared planning kernel.
- Existing external-provider integrations are not rewritten.
- No Git changes.

## In-Scope Surface to Inspect

- `planning profile registry/kernel`
- `external provider architecture evidence`
...
1. Planning is composition-oriented.
2. Custom code only for demonstrated gaps/adapters.
3. Ownership/failure/version risk explicit.
4. Not a Greenfield fallback.

## Negative Constraints

- No provider SDK expansion.
- No custom execution loop.
- No component installation/configuration here.
- No Git mutation.
...
If implementation and required validation pass, write the Implementer Report, create the single card checkpoint commit through the execution harness, verify the checkpoint succeeded, then read `WIR12` from the repository and continue without card-local Operator approval. Do not preload later cards. Stop only for a real blocker or unrecoverable required acceptance failure.
