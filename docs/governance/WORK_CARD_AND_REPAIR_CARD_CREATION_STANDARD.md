...
7. **Define forbidden changes**
   ...
   It must prohibit out-of-scope mechanisms: hidden state, alternate workflow routes, direct final writes, schema changes, Git mutation, fallback routing, provider SDKs, unrelated rewrites, etc., as applicable.
...
11. **State Git scope explicitly**

    The Operator is the only authority. An explicit prohibition such as `no Git this turn` constrains the Implementer and must be obeyed. When the Operator directs Git work, the card may record the required or allowed operations as ordinary task scope; that wording does not create a machine permission grant.
...
8. **Forbid broad redesign**
   ...
   The Implementer must not introduce new workflow models, hidden state, alternate persistence, schema drift, fallbacks, or unrelated UI changes unless the Operator explicitly directs them.
...
12. **Return to the original workflow**
    ...
    A repair should return the system to the parent workflow path after correction. It should not create a new workflow unless the Operator explicitly directs one.
...
## Operator authority, task scope, and MCP

The human Operator is the only authority over whether work may be performed. Work Cards and Repair Cards are bounded execution contracts: they record task scope, instructions, constraints, and evidence, but they cannot serve as permission principals.

The Implementer must follow the Operator's current direction and every explicit task constraint. In particular, `no Git this turn` or another Git prohibition must be obeyed. When Git operations are part of the task, a card may describe the required or allowed operations in ordinary prose as scope. No heading, token list, metadata field, or card format becomes a machine permission grant.

MCP does not read or interpret a card to decide task intent or scope. The Agent Harness enforces only its execution-security boundary: the `files.write` OAuth scope for mutation where authentication is configured, exact registered workspace identity and containment, Git-backed workspace requirements, published bounded action schemas, and each fixed Git command's deterministic safety checks. Operator-directed maintenance or publication work does not require a fabricated workflow phase, canonical card, or application-owned Git permission record before those bounded MCP actions can execute.
