...
const mountedRuntimeRoot = path.join(repositoryRoot, "tmp", "mounted-electron-runtime", "wc01-repair01");
...
app.whenReady().then(async () => {
  try {
    ...
    if (!restart) {
      ...
      const generatedPacket = await window.webContents.executeJavaScript(
        `window.champCity.ensureArchitectTaskPacket()`,
        true,
      );
      assert.equal(generatedPacket.ok, true, generatedPacket.errorMessages?.join("\n"));
...
function prepareFixture() {
  ...
  writeArtifact({
    ...
    data: {
      workCardId: repairId,
      workCardKind: "repair",
      rootCandidateArtifactId: `${projectId}/${phaseId}/work_card/${workCardId}`,
      parentWorkCardArtifactId: `${projectId}/${phaseId}/work_card/${workCardId}`,
      repairSequence: 1,
      triggerArtifactId: `${projectId}/${phaseId}/architect_review/${workCardId}`,
      authorizingArtifactId: `${projectId}/${phaseId}/architect_review/${workCardId}`,
      repairWorkCardArtifactId: `${projectId}/${phaseId}/work_card/${repairId}`,
      expectedImplementerReportArtifactId: `${projectId}/${phaseId}/implementer_report/${repairId}`,
...
function writeRegistry() {
  const artifacts = collectCanonicalArtifacts();
  ...
  fs.mkdirSync(path.dirname(jsonPath), { recursive: true });
  fs.writeFileSync(jsonPath, `${canonicalPrettyStringify(registryArtifact)}\n`, "utf8");
  fs.writeFileSync(markdownPath, renderArtifactMarkdown(registryArtifact), "utf8");
}

function collectCanonicalArtifacts() {
    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      try {
        ...
        if (
          ...
          artifact.schemaVersion === "champcity.artifact.v1" &&
          artifact.projectId === projectId &&
          artifact.artifactType !== "artifact_registry"
        ) {
